...
class _BingoPageState extends State<BingoPage> with WidgetsBindingObserver {
  late List<int> knockCode;

  _BingoPageState() {
    Random random = Random();
    knockCode = List.generate(8, (index) => random.nextInt(4));
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    setState(() {
      if (AppLifecycleState.inactive == state) {
        context.read<BingoRepository>().hideBingo();
      } else if (AppLifecycleState.resumed == state) {
        context.read<BingoRepository>().showBingo(knockCode);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    final prev0 = context.watch<BingoRepository>().prev0;
    final prev1 = context.watch<BingoRepository>().prev1;
    final prev2 = context.watch<BingoRepository>().prev2;
    final isSyncing = context.watch<BingoRepository>().isSyncing;
    final isLoaded = context.watch<BingoRepository>().isLoaded;
        ...
        child: ListView(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () {
                if (!isSyncing) {
                  context.read<BingoRepository>().startDatabase(knockCode);
                }
                showKnockCode(context);
              },
            ),
            if (isSyncing) ListTile(
              title: const Text('Unpair Roku TV'),
              onTap: () {
                context.read<BingoRepository>().stopDatabase();
              },
            ),
            ListTile(
      ...
      body: SizedBox(
        child: Center(
          child: Column(
            ...
            children: <Widget>[
              if (!isLoaded && isSyncing) const Expanded(
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
              ...
              if (isLoaded || !isSyncing) Expanded(
                child: GestureDetector(
                  behavior: HitTestBehavior.translucent,
                  onTap: () {
                    context.read<BingoRepository>().incrementBingo();
                  },
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ElevatedButton(
                      onPressed: () {
                        context.read<BingoRepository>().decrementBingo();
                      },
                    ...
                    Padding(
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          context.read<BingoRepository>().resetBingo();
                        },
  ...
  void showKnockCode(BuildContext context) {
